...
#define BRK1_RAW_MAX   2700
#define BRK2_RAW_MIN   0
#define BRK2_RAW_MAX   1500
...
/* ===================== HVA FILTER ===================== */
#define HVA_LOSS_DELAY_MS 200
bool hva_raw = false;        // รับค่าตรงจาก CAN
bool hva = false;            // ค่าที่ผ่าน debounce
bool hva_prev = false;
uint32_t hva_zero_start_ms = 0;
...
void CAN_Rx_Task(void *pv) {
  ...
  while (1) {
    if (twai_receive(&rx, portMAX_DELAY) == ESP_OK) {
      if (rx.identifier == CAN_ID_HVA) {
        hva_raw = rx.data[0];
      }
      else if (rx.identifier == CAN_ID_OPMODE) {
        opmode = rx.data[0];
      }
    }
  }
...
void updateHVA() {
  ...
  if (hva_raw) {
    ...
    if (hva_zero_start_ms == 0) {
      hva_zero_start_ms = now;
    } else if (now - hva_zero_start_ms >= HVA_LOSS_DELAY_MS) {
      hva = false;
    }
  }
}
